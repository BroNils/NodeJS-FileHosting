...
app.get('/', csrfProtection, function(req, res) {
    res.render('index', {
        name: Math.floor(Math.random() * 900000) + 100000,
...
app.post('/sendfile', parseForm, csrfProtection, function(req, res) {
    if (!req.files.files) {
        ...
        if (!req.body.name) {
            return res.status(400).send('Username undefined.');
        } else if (!fi.name) {
            return res.status(400).send('Filename undefined.');
        } else if (!Number.isInteger(parseInt(req.body.name))) {
            return res.status(500).send('Username invalid');
        } else if (fi.data.length > 50000000) {
            return res.status(500).send('File is too big.');
        }
        if (!fs.existsSync('./uploads/' + req.body.name.replace(/\.\.\//g, '-').replace(/\//g, '-'))) {
            fs.mkdirSync('./uploads/' + req.body.name.replace(/\.\.\//g, '-').replace(/\//g, '-'))
        }
